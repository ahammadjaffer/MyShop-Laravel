...
<body>
    <div class="container">
        <div class="registerbox">
            <form action="/registerShop" method="post" enctype="multipart/form-data">
            <input type="hidden" name="_token" value="<?php echo csrf_token();?>">
                <h2 class="heading">Register</h2>
                <input type="text" class="form-control" name="txtshopName" id="txtshopName" placeholder="Shop Name" autofocus="autofocus"/><br>
                <input type="text" class="form-control" name="txtOwnername" id="txtOwnername" placeholder="Ownername"/><br>
                <?php if(isset($msg)){?>
   ...
   <script type="text/javascript">
        ...
        $('#selState').change(function()
        {
            ...
            if(stId)
            {
                $.ajax({
                    type:"GET",
                    url:"/api/get-districts?state_Id="+stId,
        ...
        $('#selDistrict').on('change', function()
        {
            ...
            if(districtId)
            {
                $.ajax({
                    type:"GET",
                    url:"/api/get-places?district_Id="+districtId,
                    success:function(res)
                    {
                        if(res)
                        {
